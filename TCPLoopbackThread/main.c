...
void
enqueue_outputs(T_NET_BUF *output)
{
	struct ether_packet_t *node;

	node = (struct ether_packet_t *)malloc(sizeof(struct ether_packet_t));
	if (node == NULL) {
		assert(false);
		return;
	}
	...
	loc_cpu();
	queue_insert_prev(&output_packets, &node->queue);
	unl_cpu();
}
...
reply()
{
	...
	node = (struct ether_packet_t *)packet;

	ercd = if_loop_output(node->output);
	if (ercd == E_TMOUT) {
		loc_cpu();
		queue_insert_next(&output_packets, packet);
		unl_cpu();
		return;
	}
	assert(ercd == E_OK);
...
void task3(void *arg)
{
	while (g_script < g_script_end) {
		...
		switch (c) {
		case 0:
			if (g_script >= g_script_end)
				g_task1_send = 256;
			else {
				g_task1_send = *g_script;
				g_script++;
			}
			wup_tsk(TASK1);
			break;
		...
		case 6:
			...
			break;
		case 7:
			chg_pri(TSK_SELF, TASK1_PRIORITY + 1);
		...
		case 8:
			if (g_script >= g_script_end)
				...
				tmout = ((RELTIM)*g_script);
				g_script++;
			}
			raise_soft_int(tmout, NULL);
			break;
		}
...
void clean_packets(QUEUE *packets)
{
	while (!queue_empty(packets)) {
		struct ether_packet_t *node;
		node = (struct ether_packet_t *)queue_delete_next(packets);
		free(node);
	}
}
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
	...
	ignore_syscall_assertion = 0;
	g_script = (const char *)data;
	g_script_end = &g_script[size];

	queue_initialize(&output_packets);
