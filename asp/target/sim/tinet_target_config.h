...
#define NUM_IP6_DAD_COUNT	0	/* 重複アドレス検出で送信する近隣要請の回数、		*/
								/*  0 を指定すると、重複アドレス検出を行わない。	*/
#define NUM_ND6_CACHE_ENTRY	0	/* 近隣キャッシュのエントリ数			*/
...
#define NUM_IF_RX62N_TXBUF		2	/* 送信バッファ数			*/
#define NUM_IF_RX62N_RXBUF		2	/* 受信バッファ数			*/
#define IF_RX62N_BUF_PAGE_SIZE	1518	/* バッファサイズ */

#define TMO_IF_RX62N_GET_NET_BUF	1000	/* [us]、受信用 net_buf 獲得タイムアウト	*/
					/* [s]、 送信タイムアウト			*/
...
#define TMO_IF_BTUSB_GET_NET_BUF	1	/* [ms]、受信用 net_buf 獲得タイムアウト	*/
					/* [s]、 送信タイムアウト			*/
...
#define INHNO_IF_RX62N_TRX	INT_ETH_EINT	/* パケット送受信 */
#define INTNO_IF_RX62N_TRX	INT_ETH_EINT	/* パケット送受信 */
#define INTATR_IF_RX62N_TRX	(TA_NULL)	/* 割込み属性	*/
#define INTPRI_IF_RX62N_TRX	(-1)		/* 割込み優先度	*/
